...
Item {
  ...
  function appendWebSearchFallback() {
    ...
    // Only offer the fallback while the toolbox.web-search plugin is installed.
    var searchEntry = root.item("toolbox-web-search")
    if (!searchEntry || !root.isVisible(searchEntry)) return
  ...
  // Rows this plugin adds (Ask AI, Search Web, the Apps override) ship inside
  // the plugin, so the menu works without editing the user extension file.
  property string pluginMenuPath: root.pluginFile("menu.jsonc")
  ...
  function rebuildItemsFromSources() {
    var mergedMenu = MenuModel.mergeMenuSources(root.defaultMenuItems.concat(root.pluginMenuItems), root.userMenuItems)
  ...
  function activateIndex(index, fromPointer) {
    ...
    if (row.itemId === "toolbox-web-search") {
      var searchPrefix = "Search Web: "
      var searchQuery = row.label.indexOf(searchPrefix) === 0 ? row.label.substring(searchPrefix.length) : ""
      root.opened = false
      root.filterText = ""
      Util.execArgv(["omarchy-shell", "shell", "summon", "toolbox.web-search", JSON.stringify({ query: searchQuery })])
      return
    }
    if (row.kind === "menu" || row.kind === "link") {
      ...
      opened = false
      filterText = ""
      // root.appLibrary is null on this cloned plugin (see the "programs"
      // provider comment above), so fall back to the row's own action —
      // the "programs" provider fills it with the gtk-launch command.
      if (root.appLibrary) root.appLibrary.launch(appId, label)
      else if (row.action) root.runAction(row.action)
    } else {
