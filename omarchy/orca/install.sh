#!/usr/bin/env bash
# Install this folder as the toolbox.orca Omarchy shell plugin and put it in
# the bar, right after the workspaces widget when there is one.
set -euo pipefail
...
plugin_files=(manifest.json Panel.qml Model.js snapshot.sh focus.sh README.md)
...
for cmd in jq hyprctl omarchy omarchy-shell; do
...
backup="$HOME/.local/state/toolbox-orca/backups/$(date +%Y%m%d-%H%M%S)"
mkdir -p "$backup"
...
if ! jq -e --arg id "$plugin_id" '[.. | objects | select(.id? == $id)] | length > 0' "$shell_json" >/dev/null 2>&1; then
  if jq -e '[.bar.layout.left[]?.id] | index("guibs.workspaces")' "$shell_json" >/dev/null 2>&1; then
    placement=(left --after guibs.workspaces)
  else
    placement=(left)
  fi
fi
...
printf 'Orca widget installed in %s\nBackup: %s\n' "$target" "$backup"
