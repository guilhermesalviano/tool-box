...
// anchor against.
//
// toolbox.calendar: days with events carry a dot, clicking a day selects it,
// and the selected day's events are listed under the month. Clicking an
// event opens its meeting link, or that day in Google Calendar.
Panel {
  id: root
  ...
  // ---- Events, owned and synced by the host widget.
  readonly property var events: hostWidget ? hostWidget.events : []
  readonly property var dayCounts: Events.dayCounts(events, weeks)
  property string selectedKey: todayKey
  readonly property var selectedEvents: Events.eventsForDay(events, selectedKey)
  readonly property int maxListedEvents: 6
  readonly property var syncStatus: Events.syncStatus(hostWidget ? hostWidget.rawEvents : null, clock.date.getTime())
  readonly property string selectedLabel: {
    ...
    if (selectedKey === Model.keyForDate(tomorrow)) return "TOMORROW"
  ...
  function eventTime(event) {
    ...
    var startKey = Model.keyForDate(start)
    var from = startKey === selectedKey ? Qt.formatTime(start, "HH:mm") : "…"
    var to = Model.keyForDate(end) === selectedKey || event.endMs === event.startMs ? Qt.formatTime(end, "HH:mm") : "…"
    return event.endMs === event.startMs ? from : from + "–" + to
  }

  function openEvent(event) {
    Quickshell.execDetached(["omarchy-launch-browser", Events.eventUrl(event)])
    root.close()
  }
  ...
  function open() {
    refresh()
    if (root.hostWidget && typeof root.hostWidget.sync === "function") root.hostWidget.sync()
    ...
    PanelKeyCatcher {
      ...
      Flickable {
        ...
        Column {
                  ...
                  Repeater {
                    ...
                    Rectangle {
                      ...
                      color: dayMouse.containsMouse
                        ? Style.hoverFillFor(root.contentForeground, Color.accent)
                        : (selected && !modelData.today ? Style.selectedFillFor(root.contentForeground, Color.accent) : "transparent")
                      ...
                      // One dot for any number of events: the list below says how many.
                      Rectangle {
                        visible: (root.dayCounts[modelData.key] || 0) > 0
                        anchors.horizontalCenter: parent.horizontalCenter
          ...
          // ---- The selected day's events, under the grid they belong to.
          Item {
            width: parent.width
            height: eventsBlock.height + Style.space(6)
            ...
            Column {
              ...
              Item {
                width: parent.width
                height: Math.max(dayHeading.implicitHeight, statusLabel.implicitHeight) + Style.space(6)
                ...
                Text {
                  ...
                  text: root.hostWidget && root.hostWidget.syncing ? "Syncing…" : root.syncStatus.text
                  color: root.syncStatus.problem ? Color.accent : Qt.darker(root.contentForeground, 1.9)
                  font.family: root.contentFontFamily
                  font.pixelSize: Style.font.caption
                }
              }
              ...
              Repeater {
                model: root.selectedEvents.slice(0, root.maxListedEvents)

                Rectangle {
                  required property var modelData
                  width: eventsBlock.width
                  height: Math.max(eventTitle.implicitHeight, Style.space(20)) + Style.space(12)
                  radius: Style.cornerRadius
                  color: eventMouse.containsMouse ? Style.hoverFillFor(root.contentForeground, Color.accent) : "transparent"
                  ...
                  Text {
                    ...
                    elide: Text.ElideRight
                    text: (modelData.link ? "󰕧  " : "") + modelData.calendar
                    color: Qt.darker(root.contentForeground, 1.9)
                    font.family: root.contentFontFamily
                    font.pixelSize: Style.font.caption
                  }
                  ...
                  PanelToolTip {
                    ...
                    text: modelData.link ? "Join meeting" : (modelData.location || "Open in Google Calendar")
                    fontFamily: root.contentFontFamily
                  }
                }
              }
              ...
              Text {
                visible: root.selectedEvents.length > root.maxListedEvents
                textFormat: Text.PlainText
                width: parent.width
                leftPadding: Style.space(8)
                text: "+" + (root.selectedEvents.length - root.maxListedEvents) + " more"
                color: Qt.darker(root.contentForeground, 1.9)
                font.family: root.contentFontFamily
                font.pixelSize: Style.font.caption
              }
